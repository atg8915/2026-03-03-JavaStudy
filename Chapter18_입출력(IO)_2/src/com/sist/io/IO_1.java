 ...
 *            구분자가 없다 => | , 
 *            C001,user001,pw001김민수minsu.kim1@test.com010-1000-0001서울2023-01-02VIP
              => 배열 / 컬렉션 이용 
              => 오라클 : 구분 (변수별 구분) , 이미지파일 => 보안 
                 => 사용하기 편리하다 
              => 임시 저장 
       파일 정보 확인 
       -----------
         File 
          = 생성자 
            new File("경로명/폴더명") ==> 폴더 정보
            new File("경로명/폴더명/파일")==> 파일 정보 
          = 파일명 : getName() , getPath()
                   => 파일명     => 경로+파일명 
          = 경로명 : getParent() 
          = 파일 크기 : length() => long
